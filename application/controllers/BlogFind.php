...
class BlogFind extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($searchWord != '') {
            $this->session->set_userdata('BlogSearchWord', $searchWord);
            $data['all'] = $this->CountFindBlog($searchWord);
            $pages = (int)ceil($data['all'] / 5);
            $data['pages'] = $pages - 1;
            $data['result'] = $this->findBlogs($this->session->userdata('BlogSearchWord'), 5, 0);
        ...
        } elseif ($this->session->has_userdata('BlogSearchWord')) {
            $data['all'] = $this->CountFindBlog($this->session->userdata('BlogSearchWord'));
            $pages = (int)ceil($data['all'] / 5);
            $data['pages'] = $pages - 1;
            $data['result'] = $this->findBlogs($this->session->userdata('BlogSearchWord'), 5, 0);
            $this->load->view('web-site/blog/find-result', $data);
    ...
    public function Details($passed_url = '')
    {
        if (strtoupper($passed_url) == 'INDEX' or $passed_url == '' or $passed_url == 'blogs') {
            ...
            $data['result'] = $this->findBlogs($this->session->userdata('BlogSearchWord'), 5, $data['page_id'] * 5);
            $this->load->view('web-site/blog/find-result', $data);
        }
    }
}