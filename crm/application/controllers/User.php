...
class User extends CI_Controller
{
    ...
    public function manage_news_letter()
    {
        if ($this->session->userdata('id') != "" and $this->session->userdata('status') == "1" and $this->session->userdata('type') == "1") {
            ...
            $data['DisableCheck'] = $this->User_Model->disableNewsLeadsCheck();
    ...
    public function SendNewsLetterSubmit()
    {
        ...
        $this->User_Model->insertNewsLetterContext($subject,$comment,$attachment_file);
        switch ($SourceEnquiry):
            case 'Dubai':
                $Clients = $this->User_Model->GetNewsLettersInfo('Dubai');
                break;
            case 'Normal':
                $Clients = $this->User_Model->GetNewsLettersInfo('Normal');
                break;
            default:
                $Clients = $this->User_Model->GetNewsLettersInfo('All');
                break;
        endswitch;
        foreach ($Clients as $clientsRow):
            $this->User_Model->UpdateLeadsNewsLetterStatus($clientsRow->Lead_id,1);
        endforeach;
        $i=0;
        foreach ($Clients as $clientsInfo):
            ...
            if ($attachment_file){
                $this->email->attach('./uploads/mailAttachment/'.$attachment_file);
            }
            $this->email->to($clientsInfo->email);
            $this->email->from('admin@primepropertyturkey.com');
            $this->email->subject($clientsInfo->first_name.' '.$clientsInfo->second_name.' | '.$subject);
        ...
        endforeach;
        redirect(base_url()."User/manage_news_letter");
    }
